...
function displayUsers(svg, data) {
    ...
    //Create a global node table
    var nodes = new Array();
    data.forEach(function (d, i) {
        nodes.push({"name": d.name, "size": d.count});
        nodeTable[d.name] = nodes[i];
    });
...
function buildGraph() {
    svgfdg.selectAll("g").remove();
    svgfdg.selectAll("line").remove();
    if (d3.select("#listtype").node().value !== "fdg"){
        return;
    } 
    var selectedGroups = [];
    d3.selectAll("text.user[select_state=selected]").each(function () {
        selectedGroups.push(d3.select(this).node().parentNode);
    });
    ...
    var links = new Array();
    var linkTable = {};
    // First we build the pairs then we make them target source pairs so we avoid duplicates
    selectedGroups.forEach(function (d, i) {
        var color = d3.select(d).select("rect.user").attr("color");
        var thisGroup = d3.select(d).datum();
        var list = nodes.filter(function (dd) {
            return dd.name === thisGroup.name;
        });
        if (list.length < 1) {
            nodes.push(nodeTable[thisGroup.name]);
        }
        thisGroup.links.forEach(function (dd, j) {
            var source;
            var target;
            if (dd.link in nodeTable) {
                var list = nodes.filter(function (ddd) {
                    return ddd.name === dd.link;
                });
                if (list.length < 1) {
                    nodes.push(nodeTable[dd.link]);
                }
                if (thisGroup.name !== dd.link) {
                    if (!(thisGroup.name in linkTable) && !(dd.link in linkTable)) {
                        source = thisGroup.name;
                        target = dd.link;
                        linkTable[source] = [];
                    } else if ((thisGroup.name in linkTable) && (dd.link in linkTable)) {
                        source = thisGroup.name;
                        target = dd.link;
                        linkTable[source] = [];
                    } else if (dd.link in linkTable) {
                        source = dd.link;
                        target = thisGroup.name;
                    } else {
                        source = thisGroup.name;
                        target = dd.link;
                    }
                    var list = linkTable[source].filter(function (d) {
                        return d === target;
                    });
                    if (list.length === 0 && source !== target) {
                        linkTable[source].push(target);
                        links.push({"source": nodeTable[thisGroup.name],
                            "target": nodeTable[dd.link], "count": dd.count, "color": color});
//                    edges.push({"name": dd.link, "count": dd.count});
//                    nodes[nodeTable[d.name]]["edges"] = edges;
                    }
                }
            }
        });
    ...
    var force = d3.layout.force()
            ...
            .linkDistance(100)
            .gravity(0.2)
            .size([parseInt(d3.select("#fdgdiv").style("width")), parseInt(d3.select("#fdgdiv").style("height"))])
            .nodes(nodes)
            .links(links)
            .start();
    var link = svgfdg.selectAll(".link")
            ...
            .style("stroke-width", function (d) {
                return 1;
//                return (lineScaleFactor(parseInt(d.count)));
            });
    var nodegroup = svgfdg.selectAll(".nodeg")
            .data(nodes)
            .enter().append("g")
            .classed("user", true);
    ...
    nodegroup.append("text")
            ...
            .attr("y", function () {
                return -3 - parseInt(d3.select(this.parentNode).select("circle").attr("r"));
            });
    force.on("tick", function () {
        ...
        nodegroup.attr("transform", function (d, i) {
            return 'translate(' + [nodes[i].x, nodes[i].y] + ')';
        });
